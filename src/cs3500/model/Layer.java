...
public class Layer {
   ...
   */
  public Layer(String name, RGB[][] pixels, String filterName) {
    this.name = name;
    this.image = new RGB[pixels.length][pixels[0].length];
    this.pixels = new RGB[pixels.length][pixels[0].length];
    ...
    this.filter = Filter.findByAbbr(filterName);
  }
   ...
   */
  public RGB[][] addToCanvas(RGB[][] canvas) {
    RGB[][] newCanvas = new RGB[canvas.length][canvas[0].length];
  ...
  public void copyImage() {
    for (int i = 0; i < this.pixels.length; i++) {
      for (int j = 0; j < this.pixels[0].length; j++) {
        ...
        this.pixels[i][j] = new RGB(originalRed, originalGreen, originalBlue, originalAlpha);
      }
    }
  }
  ...
  public void addImage(int x, int y, String fileName) throws IOException {
    ...
    image.readFile();
    RGB[][] imagePixels = image.getArray();
   ...
   */
  public void blendLayer(RGB[][] pixels) {
    for (int i = 0; i < pixels.length; i++) {
      for (int j = 0; j < pixels[i].length; j++) {
   ...
   * @param pixels The beneath pixels.
   */
  public void multiply(RGB[][] pixels) {
    HSL[][] currentLayer = this.getHSLArray(this.getArray());
    HSL[][] convertedLayers = this.getHSLArray(pixels);
    for (int i = 0; i < pixels.length; i++) {
      for (int j = 0; j < pixels[i].length; j++) {
   ...
   * @return HSL pixels.
   */
  public HSL[][] getHSLArray(RGB[][] pixels) {
    HSL[][] hslArray = new HSL[pixels.length][pixels[0].length];
    for (int i = 0; i < pixels.length; i++) {
      for (int j = 0; j < pixels[i].length; j++) {
   ...
   * @return RGB pixels.
   */
  public RGB[][] getRGBArray(HSL[][] pixels) {
    RGB[][] rgbArray = new RGB[pixels.length][pixels[0].length];
    for (int i = 0; i < pixels.length; i++) {
   ...
   */
  public void applyFilter(RGB[][] pixels) {
    this.copyImage();
    switch (this.filter) {
      case NORMAL: {

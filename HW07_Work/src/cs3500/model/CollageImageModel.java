...
import cs3500.misc.CollageRead;
import cs3500.misc.CollageWrite;
import cs3500.misc.FileWrite;
import cs3500.misc.PPMWrite;
...
public class CollageImageModel implements CollageModel {
  ...
  public void loadProject(String fileName) {
    ...
    this.canvas = new RGB[height][width];
    for (int i = 0; i < this.height; i++) {
      for (int j = 0; j < this.width; j++) {
        this.canvas[i][j] = new RGB(255, 255, 255, 255);
      }
  ...
  public void addLayer(String layerName) {
    Layer addLayer = new Layer(layerName, this.height, this.width);
  ...
  public int getLayer(String layerName) {
    Layer layerTemp = new Layer(layerName, this.height, this.width);
    for (int i = 0; i < this.layers.size(); i++) {
   ...
   */
  public RGB[][] getBeneathImage(int layerIndex) {
    RGB[][] tempCanvas = new RGB[this.height][this.width];
    for (int i = 0; i < this.height; i++) {
      for (int j = 0; j < this.width; j++) {
  ...
  public Filter getFilter(String filterName) {
    if (Filter.findByAbbr(filterName) == null) {
      throw new IllegalArgumentException("Filter doesn't exist.");
    }
    else {
      return Filter.findByAbbr(filterName);
    }
  }
